...
##### READING FITS ###########
	...
	def reading_fits(self, ifile, cal_flag, ac_flag, part_type):
...
####### WRITING #########
	...
	def writing_AA_strip(self, N_in, part_type, ang_type, theta_type, phi_type, ifile):
		...
		while 1:
			...
			if where_event_eq[N_event_eq-1] < len(self.Glob_event_id_test)-1:
	...
	def writing_AA_cluster(self, N_in, part_type, ang_type, theta_type, phi_type, ifile):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)
			if where_event_eq[N_event_eq-1] < len(self.Glob_event_id_cluster)-1:
				j = where_event_eq[N_event_eq-1]+1						
			else:
				break 
	...
	def writing_AA_fake(self, astrogam_version, N_in, part_type, theta_type, phi_type, ifile):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)                                          
			if where_event_eq[N_event_eq-1] < len(self.event_id_tr)-1:
				j = where_event_eq[N_event_eq-1]+1					
			else:
				break
...
######### SUMMING ENERGY AND ENERGY THRESHOLD ###########
	...
	def summing_energy(self):
		...
		while 1:           
			...
			N_event_eq = len(where_event_eq)                                          
			if where_event_eq[N_event_eq-1] < len(self.event_id_tot_temp)-1:
				j = where_event_eq[N_event_eq-1]+1					
			else:
				break
		...
		where_eth = np.where(energy_dep_tot >= self.E_th)
...
#### STRIP ANALYSIS ########
	...
	def strip_analysis(self):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)
			if where_event_eq[N_event_eq-1] < len(self.event_id_tot)-1:
				j = where_event_eq[N_event_eq-1]+1
				N_ev = N_ev + 1
			else:
				break
	...
	def L05_cluster_x(self): 		
		...
		for k in range(self.N_trig):
			...
			while 1:
				...
				if where_tray_eq_x_top[N_tray_eq_x-1] < np.size(self.Glob_tray_id_x_top[:,k])-1: 
	...
	def L05_cluster_y(self): 		
		...
		for k in range(self.N_trig):
			...
			while 1:
				...
				if where_tray_eq_y_top[N_tray_eq_y-1] < np.size(self.Glob_tray_id_y_top[:,k])-1:  
...
########## CALORIMETER ###########
	...
	def G4_cal(self):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)                                          
			if where_event_eq[N_event_eq-1] < len(self.event_id_cal)-1:
				j = where_event_eq[N_event_eq-1]+1
			else:
				break
	...
	def cal_sum(self):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)                                          
			if where_event_eq[N_event_eq-1] < len(self.event_id_tot_cal)-1:
				j = where_event_eq[N_event_eq-1]+1
			else:
				break
...
###### ANTICOINCIDENCE ######
	...
	def AC_analysis(self):
		...
		while 1:
			...
			N_event_eq = len(where_event_eq)                                          
			if where_event_eq[N_event_eq-1] < len(self.event_id_ac)-1:
				j = where_event_eq[N_event_eq-1]+1
			else:
				break
